...
Scientific software encodes algorithmic knowledge that is often absent from published papers, existing only in source code that ages and decays. We demonstrate that large language models (LLMs) can serve as archaeological instruments for legacy scientific software, reading, comprehending, and resurrecting algorithms from aging codebases. Applying this methodology to PHYLIP — the most influential phylogenetics software package, first released in 1980 — we recovered 20 algorithmic insights with cross-disciplinary connections to electrical engineering, information geometry, algebraic statistics, and coding theory. These include the recognition that Felsenstein's 1981 pruning algorithm is mathematically identical to belief propagation, published seven years before its formalization. The recovered algorithms were validated through a 36,745-line Rust reimplementation with 961 tests and zero external dependencies. Analysis of Felsenstein's software catalog revealed that 23 of 407 documented phylogenetics tools have been permanently lost. LLM-assisted code archaeology offers a scalable approach to preserving computational knowledge across the sciences.
...
**The genetic code step matrix reveals optimization for error tolerance.** PHYLIP's protein parsimony implementation constructs a 20x20 cost matrix from the genetic code, computing the minimum number of nucleotide substitutions required to convert each amino acid to every other (**Fig. 2c**). The resulting step matrix reveals that the standard genetic code places chemically similar amino acids within one nucleotide change of each other far more often than expected by chance. We computed a z-score of -2.76 against 10,000 random reassignments of amino acids to codon blocks, confirming quantitatively that the genetic code is optimized for error tolerance^8^. This connection between the Sankoff weighted parsimony algorithm and coding theory was embedded in PHYLIP's source but never published.

**LogDet is the only compositionally robust distance.** The LogDet distance^9,10^ isolates pure evolutionary signal by exploiting a determinant factorization: the divergence matrix F between two sequences factors as F = diag(pi) x P(t), and taking the log-determinant cancels the base frequency terms entirely (**Fig. 2d**). No other standard phylogenetic distance achieves this property. When base composition varies across lineages — common in bacterial genomes (GC content ranges from 25% to 75%), mitochondrial DNA, endosymbiont genomes, and ancient DNA — standard distances (JC69, K2P) produce biased estimates because they assume uniform composition across all lineages. LogDet gives correct distances regardless of compositional heterogeneity, making it essential for deep phylogenetic studies where composition has had time to diverge. Our reimplementation computes the 4x4 determinant via Laplace cofactor expansion from first principles, requiring no linear algebra library — a direct demonstration that even numerically sophisticated algorithms can be made fully self-contained.
...
To quantify the gap between PHYLIP's original algorithm designs and modern implementations, we benchmarked phylip-rs against three contemporary tools: IQ-TREE 3^11^, RAxML-NG^12^, and VeryFastTree^13^. We simulated 36 datasets under the JC69 model (10-500 taxa, 500-5,000 sites, 3 replicates per condition) and ran all tools single-threaded for fair comparison (**Fig. 4**).

On small datasets (10-20 taxa), phylip-rs found trees with log-likelihoods identical to those of IQ-TREE and RAxML-NG, with all tools often converging to the same optimum (e.g., lnL = -35,848.85 for all five tools on 20-taxon, 5,000-site datasets; normalized Robinson-Foulds distance = 0.059 for all tools). This confirms that Felsenstein's original algorithms are mathematically sound — the 1980s-era search strategy finds the same trees as modern heuristics when the search space is tractable.

At 50 taxa, the engineering gap becomes apparent: phylip-rs ML required 234 seconds versus 14 seconds for IQ-TREE (17x slower), finding trees approximately 15 log-likelihood units worse (nRF = 0.32 vs. 0.30). Beyond 100 taxa, phylip-rs ML could not complete within the 10-minute timeout, while IQ-TREE and VeryFastTree continued to scale. At the largest dataset size (500 taxa), only VeryFastTree (14 seconds) and phylip-rs NJ (180 seconds) completed; IQ-TREE and RAxML-NG also exceeded the timeout. This pattern quantifies four decades of heuristic engineering: the algorithmic foundations are identical, but modern tools employ SIMD vectorization, sophisticated tree rearrangement strategies (lazy SPR, stochastic NNI), and site-pattern-aware likelihood computation that PHYLIP's original designs lack.

Phylip-rs's neighbor-joining implementation^18^ completed all 36 datasets with competitive accuracy, demonstrating that deterministic algorithms transfer directly across implementations with only implementation-level performance differences. Memory usage was notably lower across all dataset sizes: phylip-rs used a median of 3.9 MB versus 54.5 MB for IQ-TREE (14-fold difference) and 22.1 MB for RAxML-NG, reflecting the zero-dependency architecture's minimal memory footprint. VeryFastTree, optimized for speed with approximate methods, completed all datasets in a median of 1.3 seconds and never timed out.
...
Simulated datasets were generated using a self-contained JC69 sequence simulator (no external tools). Random binary trees were constructed via iterative taxon addition with branch lengths drawn from an exponential distribution, rescaled so mean root-to-tip distance equals 0.1 substitutions per site. The dataset matrix comprised 12 conditions (10-500 taxa, 500-5,000 sites) with 3 replicates each (36 total datasets). All tools were run single-threaded: phylip-rs ML and NJ, IQ-TREE 3.0.1 (`-m JC -T 1`), RAxML-NG 1.2.2 (`--model JC --threads 1`), and VeryFastTree 4.0.5 (`-nt -nocat -threads 1`). Wall time was measured via `time.perf_counter()`, peak memory via GNU time, log-likelihood was evaluated under JC69 using phylip-rs for all inferred trees, and topology accuracy was measured as normalized Robinson-Foulds distance to the true tree. Timeout was 10 minutes (2 minutes for phylip-rs ML on datasets with 200+ taxa).
